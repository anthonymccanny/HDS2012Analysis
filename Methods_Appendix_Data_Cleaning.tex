...
\subsubsection{Handling of Advertised vs. Recommended Properties}

The HDS data structure includes both advertised homes (the property listed in the advertisement that initiated the test) and recommended homes (additional properties shown to testers by real estate agents). These are distinguished by the \texttt{HADHOME} indicator variable, where \texttt{HADHOME = 1} denotes the advertised home.

Our spatial matching assigns test scores to all properties in the long-format dataset, with each row representing a single property shown to a tester. For analyses requiring comparison of advertised versus recommended property characteristics (matching the structure of Christensen \& Timmins 2022), the data can be reshaped to wide format where each row represents a CONTROL-TESTERID pair with separate columns for advertised (\texttt{\_Ad} suffix) and recommended (\texttt{\_Rec} suffix) property test scores. For recommended properties, when multiple recommendations exist for a single tester, we use the mean test score across all recommended properties.
