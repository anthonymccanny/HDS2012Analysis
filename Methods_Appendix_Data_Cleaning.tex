...
The school matching functionality is implemented in \texttt{school\_score\_merging.R}, which provides a single function \texttt{merge\_school\_scores()} that performs all spatial matching operations. This function is called within the main data cleaning pipeline (\texttt{data\_cleaning.R}) after geocoding and ACS census data merging.
...
\subsection{GreatSchools Index and Crime Rate (C\&T Replication Merge)}

Christensen \& Timmins (2022) scraped neighborhood amenity indicators from real estate sites, including a GreatSchools index and local crime (assault) counts. Because these original sources are no longer available, we recover the variables by matching all homes in our dataset to the C\&T recommended-home file (\texttt{recsprocessed\_JPE.rds}) on \texttt{CONTROL}, \texttt{TESTERID}, and \texttt{SEQRH}. When duplicate keys appear in the replication file, we retain the first non-missing value.

This merge adds two variables to the final dataset:
\begin{itemize}
\item \texttt{Elementary\_School\_Score\_Rec}: GreatSchools index for the recommended property.
\item \texttt{Assault\_Rec}: crime/assault count for the recommended property.
\end{itemize}

Because these values are copied from the replication archive, validation focuses on coverage rather than numerical concordance. We report match rates under alternative key definitions in Table~\ref{tab:gs-crime-coverage}, generated by \texttt{matching\_diagnosis.R}.

\begin{table}[h]
\centering
\caption{GreatSchools/crime merge coverage under alternative matching rules}
\label{tab:gs-crime-coverage}
\begin{tabular}{lr}
\toprule
Matching rule & Homes matched (\%) \\
\midrule
\input{Appendix_Tables/greatschools_crime_matching_variants_rows.tex}\\
\bottomrule
\end{tabular}
\end{table}

\subsection{External Merge Validation Summary}

To standardize reporting across external datasets, we compile a single summary table of match rates, exact matches, and correlations against the Christensen \& Timmins replication data. This table is generated by \texttt{matching\_diagnosis.R} and included here as a lightweight LaTeX fragment.

\begin{landscape}
\begin{table}[p]
\centering
\small
\caption{External merge validation summary against C\&T replication data}
\begin{tabular}{lrrrrrr}
\toprule
Dataset & N (C\&T) & N matched & Match rate (\%) & Exact match (\%) & Exact match (matched \%) & Correlation \\
\midrule
\input{Appendix_Tables/matching_summary_rows.tex}\\
\bottomrule
\end{tabular}
\vspace{0.5em}
\begin{minipage}{0.95\linewidth}
\footnotesize\textit{Note:} N (C\&T) is the number of C\&T observations with a non-missing value; N matched is the number of those observations where we also have a non-missing value. Match rate (\%) = N matched / N (C\&T). Exact match (\%) is the share of C\&T observations that match exactly (within the stated tolerance). Exact match (matched \%) is the share of exact matches among the matched subset. Correlation is computed on the matched subset. For the GreatSchools/crime merge, we report coverage against our recommended-home sample and leave exact-match and correlation metrics blank because values are directly imported from the replication archive.
\end{minipage}
\end{table}
\end{landscape}

We also report a standardized coordinate alignment check comparing our geocoded recommended-property coordinates to the C\&T replication coordinates (when the geocoded file is available).

\begin{landscape}
\begin{table}[p]
\centering
\small
\caption{Geocoding alignment with C\&T coordinates}
\begin{tabular}{rrrrrrr}
\toprule
N & Mean distance (m) & Median distance (m) & Within 50m (\%) & Within 100m (\%) & Within 500m (\%) & Within 1 km (\%) \\
\midrule
\input{Appendix_Tables/coordinate_match_rows.tex}\\
\bottomrule
\end{tabular}
\end{table}
\end{landscape}
...
\textbf{Risk-Screening Environmental Indicators (RSEI):}
U.S. Environmental Protection Agency. Risk-Screening Environmental Indicators (RSEI) aggregated grid cell data and 810m standard grid shapefiles. Retrieved from https://gaftp.epa.gov/rsei/.

\textbf{Modeled PM2.5 (van Donkelaar et al., 2019):}
van Donkelaar, A., Martin, R. V., Li, C., \& Burnett, R. T. (2019). Satellite-based estimates of long-term ambient PM$_{2.5}$ concentrations (global AOD retrievals from MODIS/MISR/SeaWiFS). Data products at 0.01$^\circ$ resolution.

\section{Superfund Site Matching for Pollution Analysis}

To replicate the pollution exposure measures in Christensen and Timmins (2022), we construct counts of Superfund sites within 5 km of each test location. Superfund site locations come from the EPA National Priorities List (NPL) dataset (Excel file \texttt{epa-national-priorities-list-ciesin-mod-v2-2014.xls}). We restrict the list to sites with \texttt{NPL\_STATUS\_DATE} prior to 2012 to align with the original study period, and we retain all NPL status categories (Final, Deleted, and Proposed). Distances are computed using the Haversine formula (great-circle distance) with Earth radius set to 6,371 km. The matching is implemented in \texttt{superfund\_merging.R} and integrated into the data pipeline by adding \texttt{SFcount\_5km} to the geocoded dataset in the standardized external-merge stage (after the ACS merge).

We validate our construction against the original Christensen and Timmins replication dataset (\texttt{recsprocessed\_JPE.rds}), which provides their \texttt{SFcount} variable and the corresponding test-level coordinates (\texttt{Latitude}, \texttt{Longitude}). Using their coordinates (which are rounded to two decimal places), our baseline specification matches 92.9\% of observations exactly and 99.3\% within $\\pm 1$. We also evaluate several alternative specifications that might plausibly differ from the original implementation (e.g., different distance rules or site list cutoffs). These alternatives uniformly reduce exact match rates relative to the baseline, which supports the baseline as the best replication available given the archived data.
...
\midrule
\input{Appendix_Tables/superfund_matching_variants_rows.tex}\\
\bottomrule
\end{tabular}
\end{table}

All diagnostics reported in the table above are reproduced by \texttt{matching\_diagnosis.R}, which runs the validation on the full replication dataset and prints the match rates for the baseline and alternative specifications.

\section{RSEI Toxic Concentration (TRI) Exposure}

To measure toxic air pollution exposure from TRI facilities, we merge the EPA Risk-Screening Environmental Indicators (RSEI) aggregated grid cell data to property coordinates. We use the 810m standard grid shapefiles to align grid cells to geographic coordinates. Each grid cell is identified by the integer indices \texttt{CELLX} and \texttt{CELLY}. The aggregated RSEI files provide multiple exposure fields, including total toxic concentration (\texttt{ToxConc}), toxicity-weighted concentration (\texttt{CTConc}), non-cancer concentration (\texttt{NCTConc}), and the RSEI model score (\texttt{Score}).

Our primary exposure measure for replication is \texttt{ToxConc} from the 2012 aggregated grid (\texttt{aggmicro2022\_2012.csv}), which is closest to the study period. We match property coordinates to grid cells using nearest-centroid assignment. A polygon containment check on a large sample yields identical grid assignments to the centroid method, which is expected given the regular 810m grid. Grid cells without valid centroid coordinates are dropped prior to matching. The merge is implemented in \texttt{rsei\_merging.R} and is integrated into the pipeline after geocoding and other external merges.

\subsection{Validation Against Christensen \& Timmins}

We validate our RSEI matching by applying the same grid assignment to the exact C\&T coordinates (\texttt{recsprocessed\_JPE.rds}) and comparing our assigned values to their \texttt{RSEI} variable. Exact numeric matches are rare, which is expected when using an updated RSEI model version and updated inputs. However, the rank-order and log-scale correlations are strong. For the 2012 grid, \texttt{ToxConc} and \texttt{CTConc} produce log1p Pearson correlations around 0.92 and Spearman correlations around 0.90 with the C\&T RSEI measure. These high correlations indicate that we are matching the same spatial pattern of toxic exposure even if absolute levels differ due to model version, toxicity weighting updates, or changes in the TRI input data.

\begin{table}[h]
\centering
\caption{RSEI matching validation against C\&T replication data (2012 grid)}
\begin{tabular}{lrrrr}
\toprule
Specification & Exact match (\%) & Pearson r & log1p Pearson r & Spearman r \\
\midrule
\input{Appendix_Tables/rsei_matching_variants_rows.tex}\\
\bottomrule
\end{tabular}
\end{table}

All diagnostics reported in the table above are produced by \texttt{matching\_diagnosis.R}. We interpret the strong log1p and rank correlations as evidence that the merge correctly aligns properties with the intended RSEI exposure surface, while the weaker raw Pearson correlation reflects differences in RSEI model version and data vintage rather than a spatial matching error.

\section{Modeled PM2.5 Exposure Matching}

We merge modeled 2012 PM2.5 concentrations from the gridded van Donkelaar et al. (2019) product (V5 North America annual surface; \texttt{Data/Non\_HDS/PM2\_5/V5.NA.04.02/V5NA04.02.HybridPM25.NorthAmerica.2012001-2012364.nc}). The NetCDF file provides latitude and longitude vectors for grid-cell centers at 0.01$^\circ$ spacing in WGS84 and a \texttt{GWRPM25} grid (units: $\mu g/m^3$). Values are stored in their natural units and require no scaling. We round to one decimal to align with the Christensen \& Timmins replication values.

For each recommended property, we map its geocoded latitude and longitude to the PM2.5 grid. Our baseline approach uses nearest-neighbor assignment to the grid-cell center, implemented in \texttt{pm25\_merging.R} via the \texttt{merge\_pm25()} helper. The resulting variable is stored as \texttt{PM25\_Rec}. To assess robustness, \texttt{matching\_diagnosis.R} evaluates several alternative specifications, including floor/ceiling snapping, bilinear interpolation, and pre-rounding coordinates prior to lookup.

We validate the merge against the Christensen \& Timmins replication data (\texttt{recsprocessed\_JPE.rds}), which provides their \texttt{PM25\_Rec} values alongside coordinates. Table \ref{tab:pm25-matching} reports match rates, exact match rates, and correlations across several methodological variants (nearest, bilinear, and small coordinate shifts) applied to the V5 surface.

\begin{table}[h]
\centering
\caption{PM2.5 matching variants (V5 baseline) against C\&T replication data}
\label{tab:pm25-matching}
\begin{tabular}{lrrr}
\toprule
Specification & Match rate (\%) & Exact match (\%) & Correlation \\
\midrule
\input{Appendix_Tables/pm25_matching_variants_rows.tex}\\
\bottomrule
\end{tabular}
\end{table}

We also compare multiple available PM2.5 products from the same research group (V4.NA.02, V4.NA.03, and a MAPLE variant) using identical nearest-neighbor extraction. This comparison reveals that none of the accessible datasets replicate the C\&T values closely: exact match rates remain around 2--3\%, and correlations remain below 0.8. The best exact match among the datasets tested is the V4 MAPLE variant, while the highest correlation is achieved by the original V4.NA.02 product. These patterns strongly suggest that the original C\&T product differs from the public archives we have obtained.

\begin{table}[h]
\centering
\caption{PM2.5 dataset comparison against C\&T replication data}
\label{tab:pm25-datasets}
\begin{tabular}{lrrr}
\toprule
Dataset & Match rate (\%) & Exact match (\%) & Correlation \\
\midrule
\input{Appendix_Tables/pm25_dataset_comparison_rows.tex}\\
\bottomrule
\end{tabular}
\end{table}
